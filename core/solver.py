...
def solve(network):
    ...
    line_frequency_vars = model.addVars(
        len(network.lines),
        vtype = "I",
        lb = 0,
        name = "line_frequency"
    ...
    model.setObjective(
        gp.quicksum(direct_travelers[node_pair] for node_pair in network.traffic_demands.keys()) - 0.001 * gp.quicksum(line_frequency_vars[i] for i in range(len(network.lines))),
        sense = -1
    )

    for node_pair, demand in network.traffic_demands.items():
        model.addConstr(direct_travelers[node_pair] <= demand)
        model.addConstr(direct_travelers[node_pair] <= network.capacity * gp.quicksum(line_frequency_vars[i] for i in lines_for_path[node_pair]))

    for edge, requirement in network.line_frequencies.items():
        model.addConstr(gp.quicksum(line_frequency_vars[i] for i in lines_for_edge[edge]) >= requirement)

    model.optimize()

    if model.Status == 2:
        calculated_frequencies = [int(round(line_frequency_vars[i].X)) for i in range(len(network.lines))]
